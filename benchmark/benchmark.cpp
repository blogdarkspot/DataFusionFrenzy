...
static void BM_Container(benchmark::State& state) {
    ...
    auto bs0 = std::make_shared<dff::container::Snapshot<entry>>(c1.begin(), c1.end());
    auto bs1 = std::make_shared<dff::container::Snapshot<entry>>(c2.begin(), c2.end());

  for (auto _ : state)
  {
        auto commands = dff::algo::compute_diff<entry>(bs0, bs1);
  }
}
